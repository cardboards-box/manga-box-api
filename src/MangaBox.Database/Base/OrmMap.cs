...
public class OrmMap<T> : IOrmMapQueryable<T> where T : DbObject
{
    ...
    /// <summary>
    /// The service that executes fake Upserts SQL queries
    /// </summary>
    public readonly IFakeUpsertQueryService _fake;
    ...
    public virtual async Task<long> Upsert(T item)
    {
        ...
        {
            var (insert, update, select) = _fake.FakeUpsert<T>();
